...
# TODO: remove the map and use count instead as in https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/rds_cluster_instance#example-usage
# and here: https://dev.azure.com/dfds/Phoenix/_git/aws-modules-rds?path=/modules/db_cluster/main.tf&_a=contents&version=GBmaster
# resource "aws_rds_cluster_instance" "this" {
#   count = local.create && !local.is_serverless ? var.cluster_db_instance_count : 0

#   apply_immediately                     = var.apply_immediately
#   auto_minor_version_upgrade            = var.auto_minor_version_upgrade
#   availability_zone                     = null # var.availability_zone
...
#   engine                                = var.engine
#   engine_version                        = var.engine_version
#   identifier                            = null #var.name
#   identifier_prefix                     = "${var.name}-${count.index}-"
...
resource "aws_appautoscaling_target" "this" {
  count = var.autoscaling_enabled && !local.is_serverless ? 1 : 0
